...
@testset "tree" begin
    ...
    root = fmm.Root(mass, 4, 3)
    leaves = root.branch_system[1]
    ...
    # plot leaves
    fig_cluster = plt.figure("cluster")
    fig_cluster.clear()
    fig_cluster.add_subplot(111)
    ax = fig_cluster.get_axes()[1]
    stls = ["v", "^", ">", "<", "+", "x", "*"]
    for (i_leaf,leaf) in enumerate(leaves)
        n = length(leaf.indices)
        Xs = zeros(dims,n)
        for (i,i_index) in enumerate(leaf.indices)
            Xs[:,i] .= fmm.get_X(mass,i_index)
        end
        stl = stls[i_leaf % length(stls) + 1]
        ax.scatter(Xs[1,:], Xs[2,:], marker=stl, label="Leaf $i_leaf") #  stls[i_leaf % length(stls)],
    end

    fig_cluster.savefig("cluster_test.png")
