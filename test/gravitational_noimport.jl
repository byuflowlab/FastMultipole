...
struct Gravitational{TF}
    bodies::Vector{PointMass{TF}}
    potential::Matrix{TF}
end

function Gravitational(bodies::Matrix)
    nbodies = size(bodies)[2]
    bodies2 = [PointMass(SVector{3}(bodies[1:3,i]),bodies[4,i],bodies[5,i]) for i in 1:nbodies]
...
#Base.getindex(g::Gravitational, i, ::VectorPotential) = view(g.potential,2:4,i)
...
Base.getindex(g::Gravitational, i, ::Body) = g.bodies[i], view(g.potential,:,i)
function Base.setindex!(g::Gravitational, val, i, ::Body)
    body, potential = val
    g.bodies[i] = body
...
#function Base.setindex!(g::Gravitational, val, i, ::VectorPotential)
...
get_n_bodies(g::Gravitational) = length(g.bodies)
Base.eltype(::Gravitational{TF}) where TF = TF

buffer_element(g::Gravitational) = (deepcopy(g.bodies[1]),zeros(eltype(g),52))

#B2M!(system::Gravitational, args...) = FastMultipole.B2M!_sourcepoint(system, args...)
