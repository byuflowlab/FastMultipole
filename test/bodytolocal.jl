...
function body_to_local_point!(::Type{Point{Vortex}}, local_coefficients, harmonics::AbstractArray{TF}, Δx, strength, expansion_order::Val{P}) where {TF,P}
    ...
    for n in 1:P
        ...
        for m in 0:n
            ...
            # ϕnm
            local_coefficients[1,1,i] += _n_inv * ((n-m)*0.5 * (ωx * S_n_mp1_real - ωy * S_n_mp1_imag) - (n+m)*0.5 * (ωx * S_n_mm1_real + ωy * S_n_mm1_imag) + ωz * m * S_n_m_imag)
            local_coefficients[2,1,i] += _n_inv * ((n-m)*0.5 * (ωx * S_n_mp1_imag + ωy * S_n_mp1_real) - (n+m)*0.5 * (ωx * S_n_mm1_imag - ωy * S_n_mm1_real) - ωz * m * S_n_m_real)
    ...
    for n in 0:P
        ...
        for m in 0:n
            ...
            # χnm
            local_coefficients[1,2,i] += np1_inv * (0.5 * (ωy * S_np1_mm1_real - ωx * S_np1_mm1_imag) - 0.5 * (ωy * S_np1_mp1_real + ωx * S_np1_mp1_imag) - ωz * S_np1_m_real)
            local_coefficients[2,2,i] += np1_inv * (0.5 * (ωy * S_np1_mm1_imag + ωx * S_np1_mm1_imag) - 0.5 * (ωy * S_np1_mp1_imag - ωx * S_np1_mp1_real) - ωz * S_np1_m_imag)
