...
harmonics = zeros(2,2, harmonic_index(p,p))
ρ = 1.7
θ = 3π/8
ϕ = 5π/3
...
for n in 0:p
    for m in 0:n
        ...
        @test isapprox(real(Rnm), harmonics[1,1,harmonic_index(n,m)]; atol=1e-12)
        @test isapprox(imag(Rnm), harmonics[2,1,harmonic_index(n,m)]; atol=1e-12)
    end
end
...
for n in 0:p
    for m in 0:n
        ...
        @test isapprox(real(Snm), harmonics[1,1,harmonic_index(n,m)]; atol=1e-12)
        @test isapprox(imag(Snm), harmonics[2,1,harmonic_index(n,m)]; atol=1e-12)
    end
end
