...
struct MultiBranch{TF1,TF2,N}
    ...
    center::SVector{3,TF1}   # center of the branch
    radius::TF1              # side lengths of the cube encapsulating the branch
    multipole_expansion::Array{Complex{TF2},2} # multipole expansion coefficients
    local_expansion::Array{Complex{TF2},2}     # local expansion coefficients
    lock::ReentrantLock
    child_lock::ReentrantLock
end

struct SingleBranch{TF1,TF2}
    n_branches::Int8        # number of child branches
...
"""
struct MultiTree{TF1,TF2,N} <: Tree
    branches::Vector{MultiBranch{TF1,TF2,N}}        # a vector of `Branch` objects composing the tree
...
struct SingleTree{TF1,TF2} <: Tree
    branches::Vector{SingleBranch{TF1,TF2}}        # a vector of `Branch` objects composing the tree
    expansion_order::Int16
    n_per_branch::Int32    # max number of bodies in a leaf
    index::Vector{Int}
