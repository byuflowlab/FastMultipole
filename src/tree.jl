...
function Tree(systems::Tuple, expansion_order, n_per_branch; shrinking=true)
    ...
    TF1 = eltype(center)
    TF2 = eltype(systems[1])
    branches = Vector{MultiBranch{TF1,TF2,n_systems}}(undef,1)
    level = 0
...
function multi_branch!(branches, systems, buffer_list, index_list, buffer_index_list, i_start, i_end, i_branch, center, radius, level, expansion_order, n_per_branch)
    ...
    n_systems = length(systems)
    TF = eltype(systems[1])
    multipole_expansion = initialize_expansion(TF,expansion_order)
    local_expansion = initialize_expansion(TF,expansion_order)
...
function initialize_expansion(TF,expansion_order)
    return zeros(Complex{TF}, 4, ((expansion_order+1) * (expansion_order+2)) >> 1)
end
