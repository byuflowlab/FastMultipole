...
@inline function B2M_point!(::Type{Point{ConstantSource{scale}}}, multipole_coefficients, harmonics, system, i_body, expansion_order) where scale
    multipole_coefficients = view(multipole_coefficients,:,1,:)
    q = system[i_body,SCALAR_STRENGTH] * scale
    B2M_point_source!(multipole_coefficients, harmonics, q, expansion_order)
end

@inline function B2M_point!(::Type{Point{Vortex{scale}}}, multipole_coefficients, harmonics, system, i_body, expansion_order) where scale
    qx, qy, qz = system[i_body,VECTOR_STRENGTH]

    # x component
    coefficients = view(multipole_coefficients,:,2,:)
    B2M_point_source!(coefficients, harmonics, qx*scale, expansion_order)

    # y component
    coefficients = view(multipole_coefficients,:,3,:)
    B2M_point_source!(coefficients, harmonics, qy*scale, expansion_order)

    # z component
    coefficients = view(multipole_coefficients,:,4,:)
    B2M_point_source!(coefficients, harmonics, qz*scale, expansion_order)

end

@inline function B2M_point_source!(multipole_coefficients, harmonics, q, expansion_order::Val{P}) where P
    for l in 0:P
        for m in 0:l
            i_solid_harmonic = l*l + l + m + 1
            i_compressed = 1 + (l * (l + 1)) >> 1 + m # only save half as Yl{-m} = conj(Ylm)
            if DEBUG_TOGGLE[] && l < 2
                @show harmonics[1,i_solid_harmonic] q
            end
