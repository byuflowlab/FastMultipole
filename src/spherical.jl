function cartesian_2_spherical!(dx; EPSILON=1e-10)
    r = sqrt(dx' * dx)
    # @show r.partials
    theta = r < EPSILON ? 0.0 : (dx[3] / r == -1 ? pi*one(eltype(dx)) : acos(dx[3] / r))
    phi = r < EPSILON ? 0.0 : ((dx[2] == 0.0)&&(dx[1] == 0.0) ? zero(eltype(dx)) : atan(dx[2], dx[1]))
    dx .= r, theta, phi
end

function cartesian_2_spherical(dx; EPSILON=1e-10)
    # @show "spherical 10" "1" r.partials
    r = sqrt(dx' * dx)
    # @show "2" r.partials
    theta = r < EPSILON ? 0.0 : (dx[3] / r == -1 ? pi*one(eltype(dx)) : acos(dx[3] / r))
    phi = r < EPSILON ? 0.0 : ((dx[2] == 0.0)&&(dx[1] == 0.0) ? zero(eltype(dx)) : atan(dx[2], dx[1]))
...
function cartesian_2_spherical(x, y, z; EPSILON=1e-10)
    # @show "spherical 18" "1" r.partials
    x2y2 = x^2 + y^2
    r2 = x2y2 + z^2
    ...
    #flag = true in isnan.(r.partials)|| true in isnan.(r.partials)
    theta = iszero(r) ? zero(r) : (abs(z / r) == 1 ? pi*one(eltype(z/r)) : acos(z / r))
    #if !flag && true in isnan.(theta.partials)
   #     @show r z
    #end
...
#    flag = true in isnan.(dx[1].partials) || true in isnan.(dx[2].partials) || true in isnan.(dx[3].partials)
    r, theta, phi = cartesian_2_spherical(dx)
 #   if !flag 
  #      if true in isnan.(r.partials) || true in isnan.(theta.partials) || true in isnan.(phi.partials)
   #         @show "Nans!" r.partials theta.partials phi.partials
    #    end
    #end
...
function M2L!(target_branch, source_branch, harmonics, L, expansion_order::Val{P}) where P
    ...
    #flag = true in isnan.(dx.partials) || true in isnan.(dy.partials) || true in isnan.(dz.partials)
    r, theta, phi = cartesian_2_spherical(dx, dy, dz)
   # if !flag 
    #    if true in isnan.(r.partials) || true in isnan.(theta.partials) || true in isnan.(phi.partials)
     #       @show "Nans!" r.partials theta.partials phi.partials
